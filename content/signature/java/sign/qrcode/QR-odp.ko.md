...
############################# Head ############################
head_title: "Java의 QR QR 코드로 Odp 문서에 전자 서명"
head_description: "짧은 Java 코드와 함께 Java을(를) 사용하여 QR QR 코드를 생성하고 Odp 파일에 넣습니다. GroupDocs 문서 서명 API를 사용하여 QR 코드로 비즈니스 문서 및 파일에 전자 서명하십시오."

############################# Header ############################
title: "Java에서 Odp 문서에 대한 QR QR 코드 서명 생성"
description: "QR QR 코드로 Odp 문서 및 계약에 전자 서명합니다. QR 코드 서명을 빠르고 쉽게 생성합니다."
...
steps:
    enable: true
    title_left: "Java에서 Qrcode으로 Odp에 서명하는 단계"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/)은 Qrcode 서명으로 Odp 문서에 빠르고 쉽게 서명할 수 있는 기능을 제공합니다.
        
        * 경로 또는 메모리 스트림으로 서명해야 하는 Odp 파일을 제공하는 Signature 클래스의 인스턴스 생성
        * SignOptions 클래스를 인스턴스화하고 필요한 모든 데이터를 설정합니다.
        * 출력 Odp 파일 또는 메모리 스트림을 전달하는 Signature.Sign() 메서드를 호출합니다.
...
demos:
    enable: true
    title: "Qrcode 라이브 데모로 Odp 문서 서명"
    content: |
       지금 바로 [GroupDocs.Signature 앱](https://products.groupdocs.app/signature/family) 웹사이트에서 다양한 서명으로 Odp 파일에 서명하세요. 무료 온라인 데모가 여러분을 기다리고 있습니다.
...
more_formats:
    ...
    content: |
        "다른 서명 유형으로 Odp에 서명할 수도 있습니다. 아래 목록을 참조하십시오."
    format: 
