---
############################# Static ############################
layout: "auto-gen-signature"
date: 2022-03-01T15:12:22
draft: false
operation: Sign
signaturetype: Qrcode
codetype: G S1 Data Matrix
fileformat: Png
productName: Java
lang: is
productCode: java
otherformats: pdf doc docx docm dot dotm dotx odt ott rtf xls xlsx xlsm xlsb csv ods ots xltx xltm ppt pptx pps ppsx odp otp potx potm pptm ppsm png jpg bmp gif tiff svg webp wmf
breadcrumb: Put  Qrcode signature on Png for Java

############################# Head ############################
head_title: "eSign Png skjal með G S1 Data Matrix QR kóða í Java"
head_description: "Búðu til G S1 Data Matrix QR kóða og settu hann á Png skrá með því að nota Java með stuttu stykki af Java kóða. Notaðu GroupDocs Document Signature API til að rafrænt undirrita viðskiptaskjölin þín og skrár með QR kóða."

############################# Header ############################
title: "Búðu til G S1 Data Matrix QR kóða undirskrift fyrir Png skjal í Java"
description: "eSignaðu Png skjölin þín og samninga með G S1 Data Matrix QR kóða. Búðu til QR kóða undirskrift fljótt og auðveldlega."
bg_image: "https://cms.admin.containerize.com/templates/aspose/App_Themes/V3/images/bg/header1.png"
bg_overlay: false
button:
    enable: true

############################# SubMenu ############################
submenu:
    enable: true

    left:
        img_alt: "GroupDocs.Signature for Java"
        image: "https://cms.admin.containerize.com/templates/groupdocs/images/product-logos/90x90-noborder/groupdocs-signature-java.png"
        product: "GroupDocs.Signature"
        platform: "Java"



############################# About ############################
about:
    enable: true
    title: "Um GroupDocs.Signature for Java QR kóða undirskriftarforritaskil"
    content: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) er þroskað API til að búa til og búa til QR kóða undirskriftir fyrir skjölin. Notendur geta búið til QR kóða undirskriftir sem veita texta til að hlaða honum niður eða deila á samfélagsmiðlum sem mynd. Hægt er að skanna undirritaða skjalið með API eða einfaldlega yfir farsíma myndavélina! Skrifaðu undir rafrænt viðskiptasamninga þína og opinber skjöl með því að bæta við QR kóða undirskrift og vinna með það. Sérhver QR kóða undirskrift mun innihalda einstakar sérsniðnar upplýsingar til að auðkenna undirritaðan eða heimila skjalið. Ennfremur er hægt að dulkóða og afkóða innihald QR kóða með persónulegum lyklum forritunarlega. Það opnar marga möguleika til að deila viðkvæmum gögnum inni í opinberum skjölum. Eftir undirritun getur notandi uppfært, staðfest, eytt, forskoðað eða leitað að QR kóðanum í PDF skjölum, MS Word skjölum, MS Excel vinnubókum, MS PowerPoint kynningum, Adobe Photoshop skrám og ýmsum myndsniðum. Hægt væri að aðlaga QR kóða til viðbótar.
    

############################# Steps ############################
steps:
    enable: true
    title_left: "Skref til að undirrita Png með Qrcode í Java"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) veitir möguleika á að undirrita Png skjöl með Qrcode undirskrift fljótt og auðveldlega.
        
        * Búðu til tilvik af Signature class sem gefur upp Png skrá sem á að undirrita sem slóð eða minnisstraum
        * Upphafðu SignOptions flokkinn og stilltu öll umbeðin gögn.
        * Kallaðu á Signature.Sign() aðferðina sem sendir úttak Png skrá eða minnisstraum

    title_right: " kerfis kröfur"
    content_right: |
        GroupDocs.Signature for Java eru studd á öllum helstu kerfum og stýrikerfum. Áður en þú keyrir kóðann hér að neðan skaltu ganga úr skugga um að þú hafir eftirfarandi forsendur uppsettar á kerfinu þínu.

        * Stýrikerfi: Microsoft Windows, Linux, MacOS
        * Þróunarumhverfi: NetBeans, Intellij IDEA, Eclipse, etc.
        * Java runtime: J2SE 6.0 and above
        * Fáðu nýjasta GroupDocs.Signature for Java frá [Maven](https://repository.groupdocs.com/webapp/#/artifacts/browse/tree/General/repo/com/groupdocs/groupdocs-signature)
         
    code: |
        ```java    
                
        // Set up input Png file
        String filePath = "input.png";
        // Set up output file
        String outputFilePath = "output.png";

        // Instantiate Signature for input file
        Signature signature = new Signature(filePath);

        // create barcode option with predefined barcode text
        BarcodeSignOptions options = new BarcodeSignOptions("John Smith");

        // setup Barcode encoding type
        options.setEncodeType(QrCodeTypes.G S1 Data Matrix);

        // set signature position
        options.setLeft(50);
        options.setTop(50);
        options.setWidth(200);
        options.setHeight(50);

        // sign Png document
        SignResult result = signature.sign(outputFilePath, options);

        ```

############################# Demos ############################
demos:
    enable: true
    title: "Undirritar Png skjöl með Qrcode lifandi kynningu"
    content: |
       Skrifaðu undir Png skrána með ýmsum undirskriftum núna með því að fara á [GroupDocs.Signature App](https://products.groupdocs.app/signature/family) vefsíðuna. Ókeypis kynning á netinu bíður þín.

              
############################# About Formats ############################
about_formats:
    enable: true
    format:
        # format loop
        - icon: "fas fa-barcode"
          title: "About G S1 Data Matrix QrCode"
          content: |
            GS1 Data Matrix er afbrigði af Data Matrix sem er í samræmi við GS1 forskriftir. Notaðu GS1 DataMatrix til að kóða upplýsingarnar eins og, en takmarkast ekki við, eftirfarandi: AI(01) Global Trade Item Number (GTIN), AI(17) Fyrningardagsetning, AI(10) Lotunúmer, AI(21) Raðnúmer .
          characterset: |
             Stafasett: há- og lágstafir og sérstafir $@%*+-./:=<>;&*_,'! allt með réttum forritaauðkennum.
          textcapacity: |
             Getu kóðatexta: allt að 3116 tölustafir, eða 2335 alfa tölustafir.
          image: |
             

          link: ""

############################# More Formats ############################
more_formats:
    enable: true
    title: "Aðrar studdar Qrcode undirskriftir fyrir Java"
    content: |
        "Þú getur líka skrifað undir Png með öðrum undirskriftartegundum. Vinsamlegast skoðaðu listann hér að neðan."
    format: 
        
       
back_to_top:
    enable: true
---