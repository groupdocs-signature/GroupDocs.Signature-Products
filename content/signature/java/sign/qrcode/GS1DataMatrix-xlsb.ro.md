---
############################# Static ############################
layout: "auto-gen-signature"
date: 2022-03-01T15:12:22
draft: false
operation: Sign
signaturetype: Qrcode
codetype: G S1 Data Matrix
fileformat: Xlsb
productName: Java
lang: ro
productCode: java
otherformats: pdf doc docx docm dot dotm dotx odt ott rtf xls xlsx xlsm xlsb csv ods ots xltx xltm ppt pptx pps ppsx odp otp potx potm pptm ppsm png jpg bmp gif tiff svg webp wmf
breadcrumb: Put  Qrcode signature on Xlsb for Java

############################# Head ############################
head_title: "eSign documentul Xlsb cu codul QR G S1 Data Matrix în Java"
head_description: "Creați codul QR G S1 Data Matrix și puneți-l în fișierul Xlsb folosind Java cu o scurtă bucată de cod Java. Utilizați API-ul GroupDocs Document Signature pentru a vă semna electronic documentele și fișierele de afaceri cu codul QR."

############################# Header ############################
title: "Generați semnătura G S1 Data Matrix cod QR pentru documentul Xlsb în Java"
description: "eSemnați documentele și contractele Xlsb cu codul QR G S1 Data Matrix. Generați semnătura codului QR rapid și ușor."
bg_image: "https://cms.admin.containerize.com/templates/aspose/App_Themes/V3/images/bg/header1.png"
bg_overlay: false
button:
    enable: true

############################# SubMenu ############################
submenu:
    enable: true

    left:
        img_alt: "GroupDocs.Signature for Java"
        image: "https://cms.admin.containerize.com/templates/groupdocs/images/product-logos/90x90-noborder/groupdocs-signature-java.png"
        product: "GroupDocs.Signature"
        platform: "Java"



############################# About ############################
about:
    enable: true
    title: "Despre GroupDocs.Signature for Java API-ul pentru semnături cu coduri QR"
    content: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) este un API matur pentru crearea și generarea de semnături QR Code pentru documente. Utilizatorii pot genera semnături de coduri QR care furnizează text pentru a-l descărca sau partaja prin intermediul rețelelor sociale ca imagine. Documentul semnat poate fi scanat cu API sau pur și simplu prin camera mobilă! Semnează electronic contractele de afaceri și documentele oficiale adăugând semnătura codului QR și manipulează-le. Orice semnătură de cod QR va conține informații personalizate unice pentru a identifica semnatarul sau autorizează documentul. În plus, conținutul codului QR poate fi criptat și decriptat cu chei personale în mod programatic. Acest lucru deschide multe abilități de a partaja date sensibile în documentele publice. După ce utilizatorul semnat poate actualiza, verifica, șterge, previzualiza sau căuta codurile QR din PDF-uri, documente MS Word, registre de lucru MS Excel, prezentări MS PowerPoint, fișiere Adobe Photoshop și diferite formate de imagine. Codurile QR pot fi personalizate suplimentar.
    

############################# Steps ############################
steps:
    enable: true
    title_left: "Pași pentru a semna Xlsb cu Qrcode în Java"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) oferă posibilitatea de a semna documente Xlsb cu semnături Qrcode rapid și ușor.
        
        * Creați o instanță a clasei Signature care furnizează fișierul Xlsb care ar trebui să se semneze ca cale sau flux de memorie
        * Instanțiați clasa SignOptions și setați toate datele solicitate.
        * Invocați metoda Signature.Sign() pasând fișierul de ieșire Xlsb sau fluxul de memorie

    title_right: " Cerințe de sistem"
    content_right: |
        GroupDocs.Signature for Java sunt acceptate pe toate platformele și sistemele de operare majore. Înainte de a executa codul de mai jos, vă rugăm să vă asigurați că aveți următoarele cerințe preliminare instalate pe sistemul dumneavoastră.

        * Sisteme de operare: Microsoft Windows, Linux, MacOS
        * Medii de dezvoltare: NetBeans, Intellij IDEA, Eclipse, etc.
        * Java runtime: J2SE 6.0 and above
        * Obțineți cel mai recent GroupDocs.Signature for Java de la [Maven](https://repository.groupdocs.com/webapp/#/artifacts/browse/tree/General/repo/com/groupdocs/groupdocs-signature)
         
    code: |
        ```java    
                
        // Set up input Xlsb file
        String filePath = "input.xlsb";
        // Set up output file
        String outputFilePath = "output.xlsb";

        // Instantiate Signature for input file
        Signature signature = new Signature(filePath);

        // create barcode option with predefined barcode text
        BarcodeSignOptions options = new BarcodeSignOptions("John Smith");

        // setup Barcode encoding type
        options.setEncodeType(QrCodeTypes.G S1 Data Matrix);

        // set signature position
        options.setLeft(50);
        options.setTop(50);
        options.setWidth(200);
        options.setHeight(50);

        // sign Xlsb document
        SignResult result = signature.sign(outputFilePath, options);

        ```

############################# Demos ############################
demos:
    enable: true
    title: "Semnează documente Xlsb cu Qrcode Live Demo"
    content: |
       Semnați fișierul Xlsb cu diferite semnături chiar acum, vizitând site-ul web [GroupDocs.Signature App](https://products.groupdocs.app/signature/family). Demo online gratuită vă așteaptă.

              
############################# About Formats ############################
about_formats:
    enable: true
    format:
        # format loop
        - icon: "fas fa-barcode"
          title: "About G S1 Data Matrix QrCode"
          content: |
            GS1 Data Matrix este o variantă a Data Matrix care este conformă cu specificațiile GS1. Utilizați GS1 DataMatrix pentru a codifica informații precum, dar fără a se limita la, următoarele: AI(01) Numărul articolului comercial global (GTIN), AI(17) Data expirării, AI(10) Numărul lotului, AI(21) Numărul de serie .
          characterset: |
             Set de caractere: litere mari și mici și caractere speciale $@%*+-./:=<>;&*_,'! toate cu identificatorii de aplicație corecti.
          textcapacity: |
             Capacitate cod text: până la 3116 cifre numerice sau 2335 caractere alfanumerice.
          image: |
             

          link: ""

############################# More Formats ############################
more_formats:
    enable: true
    title: "Alte semnături acceptate de Qrcode pentru Java"
    content: |
        "De asemenea, puteți semna Xlsb cu alte tipuri de semnături. Vă rugăm să vedeți lista de mai jos."
    format: 
        
       
back_to_top:
    enable: true
---