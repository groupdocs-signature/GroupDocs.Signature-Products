...
############################# Head ############################
head_title: "在 Java 中带有 G S1 Q R QR 码的 eSign Ppsm 文档"
head_description: "创建 G S1 Q R 二维码并使用 Java 和一小段 Java 代码将其放在 Ppsm 文件中。使用 GroupDocs 文档签名 API 使用 QR 码对您的业务文档和文件进行电子签名。"

############################# Header ############################
title: "在 Java 中为 Ppsm 文档生成 G S1 Q R 二维码签名"
description: "使用 G S1 Q R 二维码对您的 Ppsm 文档和合同进行电子签名。快速轻松地生成二维码签名。"
...
steps:
    enable: true
    title_left: "在 Java 中使用 Qrcode 签署 Ppsm 的步骤"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) 提供使用 Qrcode 签名快速轻松地签署 Ppsm 文档的能力。
        
        * 创建一个 Signature 类的实例，提供 Ppsm 文件应该作为路径或内存流进行签名
        * 实例化 SignOptions 类并设置所有需要的数据。
        * 调用 Signature.Sign() 方法传递输出 Ppsm 文件或内存流
...
demos:
    enable: true
    title: "使用 Qrcode 现场演示签署 Ppsm 文档"
    content: |
       访问 [GroupDocs.Signature App](https://products.groupdocs.app/signature/family) 网站，立即使用各种签名为 Ppsm 文件签名。免费在线演示等着你。
...
more_formats:
    ...
    content: |
        "您还可以使用其他签名类型对 Ppsm 进行签名。请参阅下面的列表。"
    format: 
