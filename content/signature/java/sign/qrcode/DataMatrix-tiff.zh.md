...
############################# Head ############################
head_title: "在 Java 中带有 Data Matrix QR 码的 eSign Tiff 文档"
head_description: "创建 Data Matrix 二维码并使用 Java 和一小段 Java 代码将其放在 Tiff 文件中。使用 GroupDocs 文档签名 API 使用 QR 码对您的业务文档和文件进行电子签名。"

############################# Header ############################
title: "在 Java 中为 Tiff 文档生成 Data Matrix 二维码签名"
description: "使用 Data Matrix 二维码对您的 Tiff 文档和合同进行电子签名。快速轻松地生成二维码签名。"
...
steps:
    enable: true
    title_left: "在 Java 中使用 Qrcode 签署 Tiff 的步骤"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) 提供使用 Qrcode 签名快速轻松地签署 Tiff 文档的能力。
        
        * 创建一个 Signature 类的实例，提供 Tiff 文件应该作为路径或内存流进行签名
        * 实例化 SignOptions 类并设置所有需要的数据。
        * 调用 Signature.Sign() 方法传递输出 Tiff 文件或内存流
...
demos:
    enable: true
    title: "使用 Qrcode 现场演示签署 Tiff 文档"
    content: |
       访问 [GroupDocs.Signature App](https://products.groupdocs.app/signature/family) 网站，立即使用各种签名为 Tiff 文件签名。免费在线演示等着你。
...
about_formats:
    ...
    format:
        ...
        - icon: "fas fa-barcode"
          ...
          content: |
            数据矩阵是一种二维码，由以正方形或矩形图案排列的黑白“单元”或点组成，也称为矩阵。要编码的信息可以是文本或数字数据。
          characterset: |
             支持所有 256 个 ASCII 字符、所有 ISO 字符和所有扩展二进制编码十进制交换码 (EBCDIC) 字符。
          textcapacity: |
             最多 3116 个数字，或 2335 个字母数字字符，或 1556 个二进制字符。
          image: |
...
more_formats:
    ...
    content: |
        "您还可以使用其他签名类型对 Tiff 进行签名。请参阅下面的列表。"
    format: 
