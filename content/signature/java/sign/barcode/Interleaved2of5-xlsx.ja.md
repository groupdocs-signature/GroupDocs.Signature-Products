...
############################# Head ############################
head_title: "Java で Interleaved2of5 バーコードを使用して Xlsx 文書に eSign"
head_description: "Interleaved2of5 バーコード署名を作成し、数行のコードを使用して Java で Xlsx ドキュメントに配置します。 GroupDocs Document Signature API を使用して、さまざまなファイル形式に署名します。"

############################# Header ############################
title: "Java の Xlsx ドキュメントの Interleaved2of5 バーコード署名を生成します"
description: "Xlsx ビジネス文書に Interleaved2of5 バーコードで電子署名します。署名オプションを設定するための数行のコードで、バーコード署名をすばやく簡単に生成します。"
...
steps:
    enable: true
    title_left: "Java で Barcode を使用して Xlsx に署名する手順"
    content_left: |
        [GroupDocs.Signature for Java](https://products.groupdocs.com/signature/java/) は、Xlsx ドキュメントに Barcode 署名ですばやく簡単に署名する機能を提供します。
...
demos:
    enable: true
    title: "Barcode ライブ デモで Xlsx ドキュメントに署名"
    content: |
...
about_formats:
    ...
    format:
        ...
        - icon: "fas fa-barcode"
          ...
          content: |
            Interleaved 2 of 5 (ITF) は、数字をエンコードする連続した 2 幅のバーコード記号です。 135 フィルム、ITF-14 バーコード、および一部の製品のカートンで商業的に使用されていますが、内部の製品には UPC または EAN のラベルが付けられています。
          characterset: |
             数字 (0 から 9)。
          textcapacity: |
             可変長。
