...
############################# Head ############################
head_title: "C# の Xls ファイルで Qrcode 署名を検索します"
head_description: "数行のコードを使用して Xls ファイル内の Qrcode 署名を検索するには、.NET を使用します。"

############################# Header ############################
title: "Xls ファイルで Qrcode 署名を検索します"
description: ".NET ネイティブ API により、署名済みの Xls ファイルで Qrcode 署名を検索できます。数行のコードを使用して、Xls ドキュメント内で高度な電子署名検索を実行します。"
...
steps:
    enable: true
    title_left: "Xls で Qrcode 署名を検索する方法"
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) を使用すると、いくつかの簡単な手順を実装することで、.NET の開発者がアプリケーションから Xls ファイル内の Qrcode 署名を簡単に検索できるようになります。
