...
head_title: ".NETのPSDファイルにデジタル署名を追加する|署名文書"
head_description: ".NETでデジタル署名を使用してPSDファイルに署名する-カスタマイズされた電子署名を人気のあるビジネスドキュメントや画像ファイル形式に追加します."

title: "PSDファイルにデジタル署名を追加する"
description: "一般的なデジタル署名タイプを使用してPSDファイルに署名します。署名プロパティを操作し、ニーズに合ったドキュメント内に事前署名オプションを設定します."
...
steps:
    enable: true
    title_left: "PSDにデジタル署名を追加する方法"
    content_left: |
        [GroupDocs.Signature](/signature/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、アプリケーション内のPSDファイルに電子署名を簡単に追加できます。
...
demos:
    ...
    content: |
        [GroupDocs.Signatureライブデモ](https://products.groupdocs.app/signature/family)サイトにアクセスして、PSDファイルに今すぐ署名を追加してください。  
        ライブデモには次の利点があります
