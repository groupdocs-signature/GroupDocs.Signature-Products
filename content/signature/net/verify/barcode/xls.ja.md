...
############################# Head ############################
head_title: "C# による Xls ファイルの Barcode 署名の検証"
head_description: "数行の .NET コードを使用して、Xls ドキュメントとその Barcode 署名を検証します。"

############################# Header ############################
title: "Xls ファイルの Barcode 署名検証"
description: ".NET の API は、Xls ドキュメントで Barcode 署名を検証する機会を提供します。 Xls ドキュメント内の電子署名の検証は、迅速かつ簡単に実行できます。"
...
steps:
    enable: true
    title_left: "Xls ドキュメントで Barcode 署名を検証する方法"
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) には、Xls ドキュメントに配置された Barcode 署名の検証などの便利な機能が含まれています。余分なコードを実装せずに、この機会を利用してください。
