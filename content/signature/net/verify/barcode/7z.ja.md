...
############################# Head ############################
head_title: "C# による 7z ファイルの Barcode 署名の検証"
head_description: "数行の .NET コードを使用して、7z ドキュメントとその Barcode 署名を検証します。"

############################# Header ############################
title: "7z ファイルの Barcode 署名検証"
description: ".NET の API は、7z ドキュメントで Barcode 署名を検証する機会を提供します。 7z ドキュメント内の電子署名の検証は、迅速かつ簡単に実行できます。"
...
steps:
    ...
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) には、7z ドキュメントに配置された Barcode 署名の検証などの便利な機能が含まれています。余分なコードを実装せずに、この機会を利用してください。
