...
############################# Head ############################
head_title: "Générez un code QR pour les fichiers PDF en utilisant C#"
head_description: "Exploitez l'API GroupDocs.Signature pour générer un code QR pour les fichiers PDF. Intégrez facilement des codes QR sur n'importe quelle page pour améliorer la fonctionnalité."
...
steps:
    enable: true
    title: "Comment générer et insérer un code QR à n'importe quel endroit d'un PDF"
    content: |
      [GroupDocs.Signature](/signature/net/) facilite la génération de codes QR dans divers formats populaires et leur placement sur les pages PDF. Supportant plus de 10 types de codes QR, notre bibliothèque peut être intégrée de manière fluide dans les applications .NET. Améliorez vos documents avec des signatures QR codes utilisant notre produit.
      
      1. Acquérez le fichier ou le flux PDF à signer avec un code QR.
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "Comment générer un code QR avec diverses options ?"
      content: |
        Cet exemple démontre comment placer un code QR personnalisé sur une page PDF.
      code:
        title: "C#"
        content: |
          ```csharp {style=abap}
