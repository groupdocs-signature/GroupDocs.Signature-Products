...
############################# Head ############################
head_title: "C#를 사용하여 PDF 파일에 QR 코드 생성"
head_description: "GroupDocs.Signature API를 활용하여 PDF 파일에 QR 코드를 생성합니다. 향상된 기능을 위해 어느 페이지에나 QR 코드를 원활하게 임베드하세요."
...
steps:
    enable: true
    title: "PDF 내 모든 장소에 QR 코드를 생성하고 삽입하는 방법"
    content: |
      [GroupDocs.Signature](/signature/net/) 은 다양한 인기 있는 형식에서 QR 코드를 생성하고 PDF 페이지에 배치할 수 있는 기능을 제공합니다. 10가지 이상의 QR 코드 유형을 지원하며, 우리의 라이브러리는 .NET 애플리케이션에 원활하게 통합할 수 있습니다. 우리 제품을 사용하여 QR 코드 서명으로 문서를 향상시켜 보세요.
      
      1. PDF 파일 또는 QR 코드로 서명할 스트림을 준비합니다.
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "다양한 옵션으로 QR 코드 생성 방법?"
      content: |
        이 예제는 PDF 페이지에 맞춤형 QR 코드를 배치하는 방법을 보여줍니다.
      code:
        title: "C#"
        content: |
          ```csharp {style=abap}
