---
############################# Static ############################
layout: "auto-gen-signature"
date: 2022-03-01T15:12:22
draft: false
operation: Sign
signaturetype: Qrcode
codetype: G S1 Q R
fileformat: Potm
productName: .NET
lang: ur
productCode: net
otherformats: pdf doc docx docm dot dotm dotx odt ott rtf xls xlsx xlsm xlsb csv ods ots xltx xltm ppt pptx pps ppsx odp otp potx potm pptm ppsm png jpg bmp gif tiff svg webp wmf
breadcrumb: Put  Qrcode signature on Potm for C#

############################# Head ############################
head_title: "C# میں G S1 Q R QR کوڈ کے ساتھ eSign Potm دستاویز"
head_description: "G S1 Q R QR کوڈ بنائیں اور اسے .NET کا استعمال کرتے ہوئے C# کوڈ کے ایک چھوٹے ٹکڑے کے ساتھ Potm فائل پر رکھیں۔ QR کوڈ کے ساتھ اپنے کاروباری دستاویزات اور فائلوں پر ای سائن کرنے کے لیے GroupDocs Document Signature API کا استعمال کریں۔"

############################# Header ############################
title: "C# میں Potm دستاویز کے لیے G S1 Q R QR کوڈ دستخط بنائیں"
description: "اپنے Potm دستاویزات اور معاہدوں پر G S1 Q R QR کوڈ کے ساتھ دستخط کریں۔ QR کوڈ دستخط جلدی اور آسانی سے بنائیں۔"
bg_image: "https://cms.admin.containerize.com/templates/aspose/App_Themes/V3/images/bg/header1.png"
bg_overlay: false
button:
    enable: true

############################# SubMenu ############################
submenu:
    enable: true

    left:
        img_alt: "GroupDocs.Signature for .NET"
        image: "https://cms.admin.containerize.com/templates/groupdocs/images/product-logos/90x90-noborder/groupdocs-signature-net.png"
        product: "GroupDocs.Signature"
        platform: ".NET"



############################# About ############################
about:
    enable: true
    title: "GroupDocs.Signature for .NET QR کوڈ دستخط API کے بارے میں"
    content: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) دستاویزات کے لیے QR کوڈ دستخط بنانے اور بنانے کے لیے ایک بالغ API ہے۔ صارفین QR کوڈ کے دستخط تیار کر سکتے ہیں جو اسے ڈاؤن لوڈ کرنے کے لیے ٹیکسٹ فراہم کرتے ہیں یا تصویر کے طور پر سوشل میڈیا پر شیئر کر سکتے ہیں۔ دستخط شدہ دستاویز کو API کے ساتھ یا صرف موبائل کیمرے پر اسکین کیا جا سکتا ہے! QR کوڈ کے دستخط شامل کرنے کے ساتھ اپنے کاروباری معاہدوں اور سرکاری دستاویزات پر الیکٹرانک طور پر دستخط کریں اور اس میں ہیرا پھیری کریں۔ کسی بھی QR کوڈ کے دستخط میں دستخط کنندہ کی شناخت یا دستاویز کی اجازت دینے کے لیے منفرد حسب ضرورت معلومات شامل ہوں گی۔ مزید برآں، کیو آر کوڈ کے مواد کو پروگرام کے لحاظ سے پرسنل کیز کے ساتھ انکرپٹ اور ڈکرپٹ کیا جا سکتا ہے۔ اس سے عوامی دستاویزات کے اندر حساس ڈیٹا کو شیئر کرنے کی بہت سی صلاحیتیں کھل جاتی ہیں۔ دستخط کرنے کے بعد صارف پی ڈی ایف، ایم ایس ورڈ دستاویزات، ایم ایس ایکسل ورک بک، ایم ایس پاورپوائنٹ پریزنٹیشنز، ایڈوب فوٹوشاپ فائلز اور مختلف امیج فارمیٹس میں کیو آر کوڈز کو اپ ڈیٹ، تصدیق، حذف، پیش نظارہ یا تلاش کر سکتا ہے۔ QR کوڈز کو اضافی طور پر اپنی مرضی کے مطابق بنایا جا سکتا ہے۔
    

############################# Steps ############################
steps:
    enable: true
    title_left: "C# میں Qrcode کے ساتھ Potm پر دستخط کرنے کے مراحل"
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) جلد اور آسانی سے Qrcode دستخطوں کے ساتھ Potm دستاویزات پر دستخط کرنے کی صلاحیت فراہم کرتا ہے۔
        
        * دستخط کلاس کی ایک مثال بنائیں جو کہ Potm فائل کو پاتھ یا میموری اسٹریم کے طور پر دستخط کرنے کے لیے فراہم کرتی ہے۔
        * SignOptions کلاس کو فوری بنائیں اور تمام مطلوبہ ڈیٹا سیٹ کریں۔
        * Signature.Sign() طریقہ پاس کرنے کے آؤٹ پٹ Potm فائل یا میموری اسٹریم کو استعمال کریں

    title_right: " سسٹم کے تقاضے"
    content_right: |
        GroupDocs.Signature for .NET تمام بڑے پلیٹ فارمز اور آپریٹنگ سسٹمز پر تعاون یافتہ ہیں۔ ذیل کے کوڈ پر عمل کرنے سے پہلے، براہ کرم یقینی بنائیں کہ آپ کے سسٹم پر درج ذیل شرائط انسٹال ہیں۔

        * آپریٹنگ سسٹم: مائیکروسافٹ ونڈوز، لینکس، میک او ایس
        * ترقی کے ماحول: Microsoft Visual Studio, Xamarin, MonoDevelop
        * Frameworks: .NET Framework, .NET Standard, .NET Core, Mono
        * تازہ ترین GroupDocs.Signature for .NET حاصل کریں [Nuget](https://www.nuget.org/packages/groupdocs.signature) سے
         
    code: |
        ```csharp    
                
        // Set up input Potm file
        string filePath = "input.potm";
        // Set up output file
        string outputFilePath = "output.potm";

        // Instantiate Signature for input file
        using (GroupDocs.Signature.Signature signature = new GroupDocs.Signature.Signature(filePath))
        {
                // create barcode option with predefined QrCode text
                QrCodeSignOptions options = new QrCodeSignOptions("JohnSmith")
                {
                    // setup QrCode encoding type
                    EncodeType = QrCodeTypes.G S1 Q R,

                    // set signature position
                    Left = 50,
                    Top = 50,
                    Width = 200,
                    Height = 50
                };

                // sign Potm document
                SignResult result = signature.Sign(outputFilePath, options);
        }

        ```

############################# Demos ############################
demos:
    enable: true
    title: "Qrcode لائیو ڈیمو کے ساتھ Potm دستاویزات پر دستخط کرنا"
    content: |
       [GroupDocs.Signature App](https://products.groupdocs.app/signature/family) ویب سائٹ پر جا کر ابھی مختلف دستخطوں کے ساتھ Potm فائل پر دستخط کریں۔ مفت آن لائن ڈیمو آپ کا منتظر ہے۔

              
############################# About Formats ############################
about_formats:
    enable: true
    format:
        # format loop
        - icon: "fas fa-barcode"
          title: "About G S1 Q R QrCode"
          content: |
            GS1 QR کوڈ QR کوڈ کا ایک متغیر ہے جو GS1 وضاحتوں کے مطابق ہے۔ یہ خاص طور پر توسیع شدہ پیکیجنگ کی معلومات، جیسے لاٹ نمبر، پروڈکٹ ID، اور مقدار کے اشتراک کے لیے ڈیزائن کیا گیا تھا۔
          characterset: |
             اپر اور لوئر کیس کے حروف اور خصوصی حروف $%*+-./: سبھی درست ایپلیکیشن شناخت کنندگان کے ساتھ
          textcapacity: |
             7089 عددی حروف تک، 4296 حروف عددی حروف
          image: |
             

          link: ""

############################# More Formats ############################
more_formats:
    enable: true
    title: "C# کے لیے دیگر تعاون یافتہ Qrcode دستخط"
    content: |
        "آپ دستخط کی دیگر اقسام کے ساتھ بھی Potm پر دستخط کر سکتے ہیں۔ براہ کرم نیچے دی گئی فہرست دیکھیں۔"
    format: 
        
       
back_to_top:
    enable: true
---