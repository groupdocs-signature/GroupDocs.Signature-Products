...
############################# Head ############################
head_title: "eSign Dcm document avec QR QR Code en C#"
head_description: "Créez le code QR QR et placez-le dans le fichier Dcm en utilisant .NET avec un court morceau de code C#. Utilisez l'API GroupDocs Document Signature pour signer électroniquement vos documents et fichiers commerciaux avec QR Code."

############################# Header ############################
title: "Générer la signature de code QR QR pour le document Dcm en C#"
description: "Signez électroniquement vos documents et contrats Dcm avec QR QR Code. Générez une signature QR Code rapidement et facilement."
...
steps:
    enable: true
    title_left: "Étapes pour signer Dcm avec Qrcode dans C#"
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) permet de signer rapidement et facilement des documents Dcm avec des signatures Qrcode.
        
        * Créez une instance de la classe Signature fournissant le fichier Dcm censé signer en tant que chemin ou flux de mémoire
        * Instanciez la classe SignOptions et définissez toutes les données demandées.
        * Appelez la méthode Signature.Sign() en transmettant le fichier de sortie Dcm ou le flux de mémoire
...
demos:
    enable: true
    title: "Signature de documents Dcm avec Qrcode Live Demo"
    content: |
       Signez dès maintenant le fichier Dcm avec différentes signatures en visitant le site Web [GroupDocs.Signature App](https://products.groupdocs.app/signature/family). Une démo en ligne gratuite vous attend.
...
about_formats:
    ...
    format:
        ...
        - icon: "fas fa-barcode"
          ...
          content: |
            QR Code (abréviation de Quick Response Code) est la marque d'un type de code-barres matriciel (ou code-barres bidimensionnel). C'est le type de code bidimensionnel le plus populaire et le plus largement utilisé. En pratique, les codes QR contiennent souvent des données pour un localisateur, un identifiant ou un tracker qui pointe vers un site Web ou une application.
          characterset: |
             Tous les 256 caractères ASCII + Kanji
          textcapacity: |
             Jusqu'à 7089 caractères numériques, 4296 caractères alphanumériques, 2953 octets (données binaires) ou 1817 caractères Kanji.
          image: |
...
more_formats:
    ...
    content: |
        "Vous pouvez également signer Dcm avec d'autres types de signature. Veuillez consulter la liste ci-dessous."
    format: 
