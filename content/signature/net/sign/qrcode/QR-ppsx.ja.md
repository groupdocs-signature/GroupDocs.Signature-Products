...
############################# Head ############################
head_title: "C# で QR QR コードを使用して Ppsx 文書に eSign"
head_description: "QR QR コードを作成し、短い C# コードを含む .NET を使用して Ppsx ファイルに配置します。 GroupDocs Document Signature API を使用して、ビジネス ドキュメントやファイルに QR コードで電子署名します。"

############################# Header ############################
title: "C# の Ppsx ドキュメントの QR QR コード署名を生成します"
description: "Ppsx ドキュメントと契約書に QR QR コードで電子署名します。 QR コード署名をすばやく簡単に生成します。"
...
about_formats:
    ...
    format:
        ...
        - icon: "fas fa-barcode"
          ...
          content: |
            QRコード（Quick Response Codeの略）は、マトリックスバーコード（または二次元バーコード）の一種の商標です。最もポピュラーで広く使われている二次元コードです。実際には、QR コードには多くの場合、Web サイトまたはアプリケーションを指すロケーター、識別子、またはトラッカーのデータが含まれています。
          characterset: |
             ASCII 全 256 文字 + 漢字
          textcapacity: |
             数字 7089 文字、英数字 4296 文字、2953 バイト (バイナリ データ) または漢字 1817 文字まで。
          image: |
