---
############################# Static ############################
layout: "auto-gen-signature"
date: 2022-03-01T15:12:22
draft: false
operation: Sign
signaturetype: Qrcode
codetype: Data Matrix
fileformat: Pdf
productName: .NET
lang: el
productCode: net
otherformats: pdf doc docx docm dot dotm dotx odt ott rtf xls xlsx xlsm xlsb csv ods ots xltx xltm ppt pptx pps ppsx odp otp potx potm pptm ppsm png jpg bmp gif tiff svg webp wmf
breadcrumb: Put  Qrcode signature on Pdf for C#

############################# Head ############################
head_title: "eSign Pdf έγγραφο με Data Matrix QR Code στο C#"
head_description: "Δημιουργήστε Data Matrix κώδικα QR και τοποθετήστε τον σε αρχείο Pdf χρησιμοποιώντας το .NET με ένα σύντομο κομμάτι κώδικα C#. Χρησιμοποιήστε το GroupDocs Document Signature API για να υπογράψετε ηλεκτρονικά τα έγγραφα και τα αρχεία της επιχείρησής σας με QR Code."

############################# Header ############################
title: "Δημιουργήστε Data Matrix υπογραφή κώδικα QR για το έγγραφο Pdf στο C#"
description: "Ηλεκτρονικά υπογράψτε τα έγγραφα και τις συμβάσεις του Pdf με τον κωδικό QR Data Matrix. Δημιουργήστε υπογραφή κωδικού QR γρήγορα και εύκολα."
bg_image: "https://cms.admin.containerize.com/templates/aspose/App_Themes/V3/images/bg/header1.png"
bg_overlay: false
button:
    enable: true

############################# SubMenu ############################
submenu:
    enable: true

    left:
        img_alt: "GroupDocs.Signature for .NET"
        image: "https://cms.admin.containerize.com/templates/groupdocs/images/product-logos/90x90-noborder/groupdocs-signature-net.png"
        product: "GroupDocs.Signature"
        platform: ".NET"



############################# About ############################
about:
    enable: true
    title: "Σχετικά με το API υπογραφών κώδικα QR GroupDocs.Signature for .NET"
    content: |
        Το [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) είναι ένα ώριμο API για τη δημιουργία και τη δημιουργία υπογραφών κώδικα QR για τα έγγραφα. Οι χρήστες μπορούν να δημιουργήσουν υπογραφές κώδικα QR παρέχοντας κείμενο για λήψη ή κοινή χρήση μέσω των μέσων κοινωνικής δικτύωσης ως εικόνα. Το υπογεγραμμένο έγγραφο μπορεί να σαρωθεί με API ή απλά μέσω της κάμερας του κινητού! Υπογράψτε ηλεκτρονικά τις επαγγελματικές σας συμβάσεις και τα επίσημα έγγραφα προσθέτοντας υπογραφή QR Code και χειριστείτε τον. Οποιαδήποτε υπογραφή κωδικού QR θα περιέχει μοναδικές προσαρμοσμένες πληροφορίες για την αναγνώριση του υπογράφοντος ή την εξουσιοδότηση του εγγράφου. Επιπλέον, το περιεχόμενο του QR Code μπορεί να κρυπτογραφηθεί και να αποκρυπτογραφηθεί με προσωπικά κλειδιά μέσω προγραμματισμού. Αυτό ανοίγει πολλές δυνατότητες για κοινή χρήση ευαίσθητων δεδομένων μέσα στα δημόσια έγγραφα. Αφού ο χρήστης που υπογράφει μπορεί να ενημερώσει, να επαληθεύσει, να διαγράψει, να κάνει προεπισκόπηση ή να αναζητήσει τους Κώδικες QR σε αρχεία PDF, έγγραφα MS Word, βιβλία εργασίας MS Excel, παρουσιάσεις MS PowerPoint, αρχεία Adobe Photoshop και διάφορες μορφές εικόνας. Οι κωδικοί QR θα μπορούσαν να προσαρμοστούν επιπλέον.
    

############################# Steps ############################
steps:
    enable: true
    title_left: "Βήματα για την υπογραφή Pdf με Qrcode στο C#"
    content_left: |
        Το [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) παρέχει τη δυνατότητα υπογραφής εγγράφων Pdf με υπογραφές Qrcode γρήγορα και εύκολα.
        
        * Δημιουργήστε ένα στιγμιότυπο κλάσης υπογραφής που παρέχει αρχείο Pdf που υποτίθεται ότι υπογράφεται ως διαδρομή ή ροή μνήμης
        * Δημιουργήστε την κλάση SignOptions και ορίστε όλα τα απαιτούμενα δεδομένα.
        * Επικαλέστε τη μέθοδο Signature.Sign() μεταβιβάζοντας την έξοδο Pdf αρχείο ή ροή μνήμης

    title_right: " Απαιτήσεις συστήματος"
    content_right: |
        Το GroupDocs.Signature for .NET υποστηρίζεται σε όλες τις μεγάλες πλατφόρμες και λειτουργικά συστήματα. Πριν εκτελέσετε τον παρακάτω κώδικα, βεβαιωθείτε ότι έχετε εγκαταστήσει τις ακόλουθες προϋποθέσεις στο σύστημά σας.

        * Λειτουργικά συστήματα: Microsoft Windows, Linux, MacOS
        * Περιβάλλοντα ανάπτυξης: Microsoft Visual Studio, Xamarin, MonoDevelop
        * Frameworks: .NET Framework, .NET Standard, .NET Core, Mono
        * Αποκτήστε το πιο πρόσφατο GroupDocs.Signature for .NET από το [Nuget](https://www.nuget.org/packages/groupdocs.signature)
         
    code: |
        ```csharp    
                
        // Set up input Pdf file
        string filePath = "input.pdf";
        // Set up output file
        string outputFilePath = "output.pdf";

        // Instantiate Signature for input file
        using (GroupDocs.Signature.Signature signature = new GroupDocs.Signature.Signature(filePath))
        {
                // create barcode option with predefined QrCode text
                QrCodeSignOptions options = new QrCodeSignOptions("JohnSmith")
                {
                    // setup QrCode encoding type
                    EncodeType = QrCodeTypes.Data Matrix,

                    // set signature position
                    Left = 50,
                    Top = 50,
                    Width = 200,
                    Height = 50
                };

                // sign Pdf document
                SignResult result = signature.Sign(outputFilePath, options);
        }

        ```

############################# Demos ############################
demos:
    enable: true
    title: "Υπογραφή εγγράφων Pdf με Qrcode Ζωντανή επίδειξη"
    content: |
       Υπογράψτε το αρχείο Pdf με διάφορες υπογραφές αυτήν τη στιγμή, μεταβαίνοντας στον ιστότοπο [GroupDocs.Signature App](https://products.groupdocs.app/signature/family). Δωρεάν online demo σας περιμένει.

              
############################# About Formats ############################
about_formats:
    enable: true
    format:
        # format loop
        - icon: "fas fa-barcode"
          title: "About Data Matrix QrCode"
          content: |
            Το Data Matrix είναι ένας δισδιάστατος κώδικας που αποτελείται από ασπρόμαυρα «κελιά» ή κουκκίδες διατεταγμένες είτε σε τετράγωνο είτε σε ορθογώνιο μοτίβο, γνωστό και ως μήτρα. Οι πληροφορίες που θα κωδικοποιηθούν μπορεί να είναι κείμενο ή αριθμητικά δεδομένα.
          characterset: |
             Υποστηρίζει και τους 256 χαρακτήρες ASCII, όλους τους χαρακτήρες ISO και όλους τους χαρακτήρες Extended Binary Coded Decimal Interchange Code (EBCDIC).
          textcapacity: |
             Έως 3116 αριθμητικά ψηφία ή 2335 αλφαριθμητικούς χαρακτήρες ή 1556 δυαδικούς χαρακτήρες.
          image: |
             

          link: ""

############################# More Formats ############################
more_formats:
    enable: true
    title: "Άλλες υποστηριζόμενες υπογραφές Qrcode για C#"
    content: |
        "Μπορείτε επίσης να υπογράψετε το Pdf με άλλους τύπους υπογραφής. Δείτε την παρακάτω λίστα."
    format: 
        
       
back_to_top:
    enable: true
---