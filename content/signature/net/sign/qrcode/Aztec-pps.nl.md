---
############################# Static ############################
layout: "auto-gen-signature"
date: 2022-03-01T15:12:22
draft: false
operation: Sign
signaturetype: Qrcode
codetype: Aztec
fileformat: Pps
productName: .NET
lang: nl
productCode: net
otherformats: pdf doc docx docm dot dotm dotx odt ott rtf xls xlsx xlsm xlsb csv ods ots xltx xltm ppt pptx pps ppsx odp otp potx potm pptm ppsm png jpg bmp gif tiff svg webp wmf
breadcrumb: Put  Qrcode signature on Pps for C#

############################# Head ############################
head_title: "eSign Pps document met Aztec QR-code in C#"
head_description: "Maak een Aztec QR-code en plaats deze in het Pps-bestand met behulp van .NET met een kort stukje C#-code. Gebruik de GroupDocs Document Signature API om uw zakelijke documenten en bestanden elektronisch te ondertekenen met QR-code."

############################# Header ############################
title: "Genereer Aztec QR Code handtekening voor Pps document in C#"
description: "Onderteken uw Pps documenten en contracten met Aztec QR-code. Genereer snel en gemakkelijk een QR-codehandtekening."
bg_image: "https://cms.admin.containerize.com/templates/aspose/App_Themes/V3/images/bg/header1.png"
bg_overlay: false
button:
    enable: true

############################# SubMenu ############################
submenu:
    enable: true

    left:
        img_alt: "GroupDocs.Signature for .NET"
        image: "https://cms.admin.containerize.com/templates/groupdocs/images/product-logos/90x90-noborder/groupdocs-signature-net.png"
        product: "GroupDocs.Signature"
        platform: ".NET"



############################# About ############################
about:
    enable: true
    title: "Over GroupDocs.Signature for .NET QR Code handtekeningen API"
    content: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) is een volwassen API voor het maken en genereren van QR-codehandtekeningen voor de documenten. Gebruikers kunnen QR-codehandtekeningen genereren met tekst om deze te downloaden of als afbeelding via de sociale media te delen. Het ondertekende document kan worden gescand met API of gewoon via de mobiele camera! Onderteken uw zakelijke contracten en officiële documenten elektronisch door een QR-codehandtekening toe te voegen en manipuleer deze. Elke handtekening met een QR-code bevat unieke aangepaste informatie om de ondertekenaar te identificeren of het document te autoriseren. Bovendien kan de inhoud van de QR-code programmatisch worden versleuteld en ontsleuteld met persoonlijke sleutels. Dat opent veel mogelijkheden om gevoelige gegevens in de openbare documenten te delen. Na de ondertekening kan de gebruiker de QR-codes bijwerken, verifiëren, verwijderen, bekijken of zoeken in PDF's, MS Word-documenten, MS Excel-werkmappen, MS PowerPoint-presentaties, Adobe Photoshop-bestanden en verschillende afbeeldingsformaten. QR-codes kunnen bovendien worden aangepast.
    

############################# Steps ############################
steps:
    enable: true
    title_left: "Stappen om Pps te ondertekenen met Qrcode in C#"
    content_left: |
        [GroupDocs.Signature for .NET](https://products.groupdocs.com/signature/net/) biedt de mogelijkheid om Pps documenten snel en gemakkelijk te ondertekenen met Qrcode handtekeningen.
        
        * Maak een instantie van de Signature-klasse die een Pps-bestand levert dat moet worden ondertekend als pad of geheugenstroom
        * Instantieer SignOptions klasse en stel alle gevraagde gegevens in.
        * Roep de Signature.Sign()-methode op en geef uitvoer Pps-bestand of geheugenstroom

    title_right: " systeem vereisten"
    content_right: |
        GroupDocs.Signature for .NET worden ondersteund op alle belangrijke platforms en besturingssystemen. Voordat u de onderstaande code uitvoert, moet u ervoor zorgen dat de volgende vereisten op uw systeem zijn geïnstalleerd.

        * Besturingssystemen: Microsoft Windows, Linux, MacOS
        * Ontwikkelomgevingen: Microsoft Visual Studio, Xamarin, MonoDevelop
        * Frameworks: .NET Framework, .NET Standard, .NET Core, Mono
        * Download de nieuwste GroupDocs.Signature for .NET van [Nuget](https://www.nuget.org/packages/groupdocs.signature)
         
    code: |
        ```csharp    
                
        // Set up input Pps file
        string filePath = "input.pps";
        // Set up output file
        string outputFilePath = "output.pps";

        // Instantiate Signature for input file
        using (GroupDocs.Signature.Signature signature = new GroupDocs.Signature.Signature(filePath))
        {
                // create barcode option with predefined QrCode text
                QrCodeSignOptions options = new QrCodeSignOptions("JohnSmith")
                {
                    // setup QrCode encoding type
                    EncodeType = QrCodeTypes.Aztec,

                    // set signature position
                    Left = 50,
                    Top = 50,
                    Width = 200,
                    Height = 50
                };

                // sign Pps document
                SignResult result = signature.Sign(outputFilePath, options);
        }

        ```

############################# Demos ############################
demos:
    enable: true
    title: "Pps documenten ondertekenen met Qrcode Live Demo"
    content: |
       Onderteken het Pps-bestand met verschillende handtekeningen op dit moment door naar de website [GroupDocs.Signature App](https://products.groupdocs.app/signature/family) te gaan. Gratis online demo voor u klaar.

              
############################# About Formats ############################
about_formats:
    enable: true
    format:
        # format loop
        - icon: "fas fa-barcode"
          title: "About Aztec QrCode"
          content: |
            Azteekse code is een tweedimensionale (2-D) matrixsymboliek voor algemene doeleinden die is ontworpen om een ​​hogere nauwkeurigheid te hebben dan andere 2D-symbolen. Een Azteeks codesymbool kan maximaal 3.832 numerieke cijfers coderen; 3.067 alfabetische tekens; of 1.914 bytes aan gegevens.
          characterset: |
             Alle 256 ASCII-tekens.
          textcapacity: |
             Tot 3.832 numerieke cijfers, of 3.067 alfabetische tekens, of 1.914 bytes aan gegevens.
          image: |
             

          link: ""

############################# More Formats ############################
more_formats:
    enable: true
    title: "Andere ondersteunde Qrcode handtekeningen voor C#"
    content: |
        "U kunt Pps ook ondertekenen met andere soorten handtekeningen. Zie de lijst hieronder."
    format: 
        
       
back_to_top:
    enable: true
---